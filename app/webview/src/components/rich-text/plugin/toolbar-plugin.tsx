import { useEditor, EditorContextType } from '@/components/rich-text/context/editor';
...
const UndoRedoContainer = ({ editor }: { editor: EditorContextType }) => {
    return (
        <div className={`flex`}>
            <Button variant={'ghost'} size={'icon'} onClick={editor.undo} disabled={!editor.canUndo}>
                <RotateCcw className={`h-4 w-4`} />
            </Button>
            <Button variant={'ghost'} size={'icon'} onClick={editor.redo} disabled={!editor.canRedo}>
...
const BlockTypeContainer = ({ editor }: { editor: EditorContextType }) => {
    ...
    const renderList = useCallback(
        (ordered: boolean) => {
            return renderText(
                <>
                    {ordered ? <ListOrdered className={'h-4 w-4'} /> : <List className={'h-4 w-4'} />}
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align='end'>
                <DropdownMenuItem onClick={editor.toggleParagraph}>{renderParagraph()}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => editor.toggleHeading('h2')}>{renderHeading2()}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => editor.toggleHeading('h3')}>{renderHeading3()}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => editor.toggleHeading('h4')}>{renderHeading4()}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => editor.toggleHeading('h5')}>{renderHeading5()}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => editor.toggleHeading('h6')}>{renderHeading6()}</DropdownMenuItem>
                <DropdownMenuItem onClick={editor.toggleBulletList}>{renderList(false)}</DropdownMenuItem>
                <DropdownMenuItem onClick={editor.toggleNumberList}>{renderList(true)}</DropdownMenuItem>
                <DropdownMenuItem onClick={editor.toggleCheckList}>{renderCheckList()}</DropdownMenuItem>
                <DropdownMenuItem onClick={editor.toggleQuote}>{renderQuote()}</DropdownMenuItem>
            </DropdownMenuContent>
        </DropdownMenu>
    );
};
