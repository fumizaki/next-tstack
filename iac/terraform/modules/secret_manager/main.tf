...
resource "aws_secretsmanager_secret" "rdb_credentials" {
  name        = "${var.environment}-${var.project_name}-rdb-credentials"
  description = "Database credentials for Next.js application"
}

resource "aws_secretsmanager_secret_version" "rdb_credentials" {
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = aws_secretsmanager_secret.rdb_credentials.arn
      },
    ]
  })
}
...
resource "aws_iam_role_policy_attachment" "secrets_manager_access" {
  role       = var.webview_task_execution_role_name
